# uv + PyTorch CUDA wheels on lab

Use this when a project on lab uses `uv` and PyTorch, especially when `uv.lock` resolves to CUDA packages that do not match lab's CUDA/driver stack.

## Observed lab baseline
...
```bash
ssh lab 'cd /path/to/project && grep -n "torch\|cu12\|cu13\|cuda\|nvidia" pyproject.toml uv.lock | head -120'
ssh lab 'readlink -f /usr/local/cuda; /usr/local/cuda/bin/nvcc --version; nvidia-smi | sed -n "1,5p"; command -v uv; uv --version'
```

If `uv.lock` contains `nvidia-*-cu13`, `cuda-toolkit 13.x`, or `torch` from PyPI with CUDA 13 dependencies while lab is CUDA 12.x, prefer resolving against a cu12x PyTorch wheel index.
