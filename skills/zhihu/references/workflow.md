...
Skill 包不携带 CLI 二进制。setup 获得用户授权后，从发布时注入的官方 HTTPS manifest 只下载当前平台版本，校验 host、文件大小、SHA-256、归档结构和二进制自报版本后安装到用户目录；不使用 sudo，也不修改 PATH。Linux 默认遵循 XDG 用户数据目录；桌面凭据使用 Secret Service，headless 使用进程级 `ZHIHU_ACCESS_SECRET`。安装协议和故障处理见 [CLI 使用文档](cli.md)。
...
- 安装、认证、完整命令、输出和错误：读取 [CLI 使用文档](cli.md)。
- Access Secret 申请、额度、术语和联系方式：读取 [开放平台指南](open-platform.md)。
- 在代码或服务中直接接入公共内容 API：读取 [HTTP API 文档](http-api.md)。
- 知识库命令、上传进度和 HTTP 字段：读取 [CLI 使用文档](cli.md) 和 [HTTP API 文档](http-api.md) 的知识库章节。
- 额度命令、公开 APIID 和 HTTP 字段：读取 [CLI 使用文档](cli.md) 和 [HTTP API 文档](http-api.md) 的额度章节。
- 开发本人或 OAuth 授权用户的创作、关注和收藏能力：读取 [用户数据 API](user-api.md)。
- 开发“知乎登录”或代表其他已授权用户访问数据：同时读取 [OAuth 应用集成](oauth.md) 和 [用户数据 API](user-api.md)。CLI 日常调用不使用 OAuth。
- 在 MCP 客户端中配置知乎现有服务：读取 [MCP 接入文档](mcp.md)。本 Skill 不建设新的 MCP Server。
