...
### 临时 UI 控件优先于根因重构
当用户明确要求“先添加一个按钮/开关/入口”等临时可操作控件时，不要顺手修复此前排查出的根因，也不要重写状态机。最小路径是：
- 复用现有公开方法或事件处理器，例如已有 `scrollToBottom` 就直接从按钮调用它
- 只改必要模板和局部样式，不新增全局状态、配置项或抽象
- 验证以相关类型检查和邻近单测为主，避免跑无关的大范围测试
- 如果依赖安装受 npm 配置影响，记录可复现的验证命令；不要把本机缺依赖当作代码问题

### Vue 聊天滚动按钮参考
- `references/hermes-web-ui-scroll-button.md` 记录了在 Vue/Naive UI 聊天列表中添加“滚动到底部”按钮的最小补丁模式和验证命令。
- `references/hermes-web-ui-virtual-scroll-bottom.md` 记录了 `vue-virtual-scroller` 长会话中按钮无效的根因：不要用 DOM `scrollHeight` 定位虚拟列表底部，应直接 `scrollToItem(lastIndex, { align: "end" })`。
