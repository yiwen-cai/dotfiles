...
```bash
# 保存到 skill 目录
cp gateway-model-probe.sh ~/.claude/skills/remote-ssh-h100/scripts/
chmod +x ~/.claude/skills/remote-ssh-h100/scripts/gateway-model-probe.sh
```
...
```bash
# 直接运行
bash ~/.claude/skills/remote-ssh-h100/scripts/gateway-model-probe.sh

# 或在 agent 中通过 terminal 调用
terminal(command="bash ~/.claude/skills/remote-ssh-h100/scripts/gateway-model-probe.sh")
```
