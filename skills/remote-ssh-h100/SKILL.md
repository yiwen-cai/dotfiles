---
name: remote-ssh-h100
description: 在远程服务器 h100（10.160.4.102）上通过 SSH 进行编码、跑实验、传输文件等工作流
user-invocable: true
metadata:
...
# Remote Server `h100` — 工作流指南

## 环境概览

| 项目 | 内容 |
|------|------|
| **主机名** | h100（`~/.ssh/config` 中配置）|

> 别名：`lab` 与 `h100` 指向同一台机器（10.160.4.102），两种写法等价（原 remote-ssh-lab 技能已并入本技能）。
...
当任务需要 SSH 到 h100 上的某个项目目录工作时，不能假设 Hermes 会自动加载远程目录中的项目指导文件。由于 `ssh h100 'cd <remote-dir> && ...'` 只是远程 shell 命令，当前 Hermes 主会话、`delegate_task` 子 agent、以及普通工具调用都不会自动注入远程目录的 `AGENTS.md` / `AGENT.md` / `CLAUDE.md` / `.cursorrules`。

**必须在执行项目任务前主动读取并遵守这些文件：**

```bash
ssh h100 'cd /path/to/project && for f in AGENTS.md AGENT.md CLAUDE.md .cursorrules; do [ -f "$f" ] && echo "===== $f =====" && sed -n "1,240p" "$f"; done; true'
```
...
```bash
ssh h100 'cd /path/to/project; for f in AGENTS.md AGENT.md CLAUDE.md .cursorrules; do [ -f "$f" ] && echo "===== $f =====" && sed -n "1,240p" "$f"; done; echo "===== next check ====="; find tests -maxdepth 1 -type f | sort'
```
...
1. 只要用户要求在 h100 的某个具体项目目录中读代码、改代码、跑测试、写文档或排查问题，就先读取该目录的指导文件。
...
# 查看进程
ssh h100 'ps aux | grep python'

# 检查 CUDA
ssh h100 'export PATH=/usr/local/cuda/bin:$PATH && nvcc --version'
```

**在 Hermes 中：** 直接通过 `terminal(command="ssh h100 '...'")` 执行。
...
```bash
# 将本地目录复制到远程
scp -r /path/to/local/project h100:~/code/

# 或 rsync（增量同步，推荐日常使用）
rsync -avz --progress /path/to/local/project h100:~/code/
```

#### 从 h100 拉取结果回本地
```bash
rsync -avz --progress h100:~/code/my-project/results/ ./local-results/
```

#### 在 Hermes 中直接编辑远程文件
```bash
# 读取远程文件（ssh + cat）
ssh h100 'cat ~/code/my-project/config.yaml'

# 写入远程文件（使用 heredoc）
ssh h100 'cat > ~/code/my-project/config.yaml << '\''EOF'\''
key: value
EOF'
```
...
```bash
# 1. 创建新 tmux 会话运行实验
ssh h100 -t 'tmux new-session -d -s my-exp && tmux send-keys -t my-exp "cd ~/code/my-project && export PATH=/usr/local/cuda/bin:\$PATH && python train.py" Enter'

# 2. 稍后检查进度
ssh h100 'tmux capture-pane -t my-exp -p -S -30'

# 3. 手动 attach 到会话查看（需要交互式终端）
ssh h100 -t 'tmux attach -t my-exp'

# 4. 实验完成后杀掉会话
ssh h100 'tmux kill-session -t my-exp'
```
...
# 查看当前有哪些进程在用 GPU
ssh h100 'nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'

# 在特定 GPU 上跑实验
ssh h100 'export PATH=/usr/local/cuda/bin:$PATH && CUDA_VISIBLE_DEVICES=0,1 python train.py'
```
...
```bash
# 查看现有镜像/容器
ssh h100 'docker images'
ssh h100 'docker ps -a'

# 拉取镜像
ssh h100 'docker pull pytorch/pytorch:latest'

# 在后台跑容器
ssh h100 'docker run -d --gpus all --name my-exp pytorch/pytorch:latest python train.py'

# 查看日志
ssh h100 'docker logs -f my-exp'

# 复制文件到容器
ssh h100 'docker cp ./data my-exp:/workspace/data'

# 交互式进入容器
ssh h100 -t 'docker exec -it my-exp /bin/bash'

# 清理
ssh h100 'docker rm -f my-exp'
```

## 多机互通与文件传输（扩展）

当需要在 h100 与其他服务器（如 a800）之间直接传输文件时，使用以下模式。

### 前置：SSH 密钥互通

默认情况下，h100 和其他服务器可能使用不同的 SSH 密钥，无法直接 SSH。

```bash
# 1. 将 h100 的公钥添加到目标机器的 authorized_keys
ssh h100 "cat ~/.ssh/id_ed25519.pub" | ssh a800 "cat >> ~/.ssh/authorized_keys"

# 2. 验证直连
ssh h100 "ssh -o StrictHostKeyChecking=no 10.160.4.104 'hostname'"
```

### 方法 A：从 h100 用 rsync 推送到其他服务器（推荐）

在 h100 上执行 rsync，利用内网高速传输：

```bash
# h100 → a800 推送
ssh h100 "rsync -avz --progress --exclude='.venv' --exclude='__pycache__' \
...
rsync -avz --progress --exclude='__pycache__' --exclude='.git' \
  ./project/ h100:~/code/project/

# h100 → 本地
rsync -avz --progress h100:~/code/project/results/ ./local-results/
```
...
Hermes 的 `terminal()` 工具可以通过 `ssh h100 '...'` 直接操作远程服务器。适合大部分场景。

```python
# Hermes 中的典型用法
terminal(command="ssh h100 'export PATH=/usr/local/cuda/bin:\$PATH && cd ~/code/my-project && python train.py --epochs 100'", timeout=600)
```
...
```python
# Step 1: 在 tmux 中启动实验
terminal(command="""ssh h100 'tmux new-session -d -s exp1 \
  "cd ~/code/my-project && export PATH=/usr/local/cuda/bin:\$PATH && CUDA_VISIBLE_DEVICES=0 python train.py"'""")

# Step 2: 一段时间后检查输出
terminal(command="ssh h100 'tmux capture-pane -t exp1 -p -S -20'")

# Step 3: 完成后清理
terminal(command="ssh h100 'tmux kill-session -t exp1'")
```
...
# Step 2: 上传到 h100
terminal(command="scp /tmp/train.py h100:~/code/my-project/")

# Step 3: 在 h100 上执行（记得加 CUDA 路径）
terminal(command="ssh h100 'export PATH=/usr/local/cuda/bin:\$PATH && cd ~/code/my-project && python train.py'")
```
...
- **`references/uv-pytorch-cuda.md`** — h100 上使用 uv 管理 PyTorch CUDA wheel 的配置模式、cu126 index 示例、验证命令和常见坑。
...
- **`references/a800-server.md`** — 另一台服务器 A800（10.160.4.104）的详细配置、SSH 设置和与 h100 互通的方法。