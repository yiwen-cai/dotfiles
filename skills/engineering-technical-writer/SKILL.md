...
## Academic technical talk / paper survey workflow

When preparing an academic technical sharing deck or literature-review presentation for this user, use `references/academic-llm-literature-review-presentations.md`. In particular, organize paper explanations as **core technical implementation principle → main advantages → limitations/applicability boundaries**, with depth adjusted by paper importance, and maintain separate `partN_explanation.md` and `partN_ppt.md` files before merging PPT descriptions.

For LLM systems talks on KV Cache optimization or reasoning-model inference, also use `references/reasoning-kv-cache-compression.md` for LongFlow-style decode-time compression framing, formulas, system caveats, and slide structure.

When the user asks for a browsable paper-note library from downloaded PDFs, use `references/pdf-to-html-paper-notes.md` to generate grounded HTML notes with PDF-rendered figures/tables, source metadata, evidence excerpts, and no fabricated data.
