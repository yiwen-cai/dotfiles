...
### 功能存在性审计（Feature-existence audit）
当用户询问“有没有某个功能”“是否支持某种交互/模式”时，按只读审计处理，而不是凭关键词或记忆回答：

1. **先找正向入口**：搜索 UI 文案、按钮、路由、命令、配置键和相关术语；同时排除 vendor/minified 结果，避免噪声。
2. **再找状态模型**：确认核心状态是单例还是按 ID / pane / tab / workspace 分组；例如检查 `S.session` / `S.messages` 这类全局状态是否为单份，还是存在 `Map` / object keyed by session 或 pane。
3. **追踪渲染目标**：确认 DOM 容器是否只有一个实例，渲染函数是否固定写入单一 `#messages` / `#msgInner`，还是能创建多个并行 view。
4. **检查切换语义**：阅读 `load*` / `switch*` / `render*` 路径，判断用户操作是覆盖当前视图、打开新窗口/标签页，还是新增并排 pane。
5. **区分近似能力**：明确区分“同页分屏”“多浏览器标签页各自打开”“后台多 session streaming”等相近但不同的能力。
6. **输出证据化结论**：如果未发现功能，不要只说“没有”；列出已检查文件、关键函数/变量/DOM id，以及为什么这些证据表明当前实现不支持该功能。
