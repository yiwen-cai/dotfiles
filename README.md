...
个人终端开发环境配置仓库，同步 Claude Code 与 Codex 的配置及 skills。
...
```bash
git clone https://github.com/drink-less-milktea/dotfiles.git ~/Documents/code/dotfiles
cd ~/Documents/code/dotfiles
./install.sh
```

脚本会把配置软链接到 `~/.claude` 和 `~/.codex`。
...
- **绝不上传**：`settings.json`、`config.json`、`auth.json`、`.env`、`*.sqlite`、`*.pem`、`id_ed25519` 等
